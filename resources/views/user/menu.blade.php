...
<section class="menu-section-area {{ Auth::check() ? 'isLoginMenu' : '' }}">
    ...
    <nav class="navbar navbar-expand-lg" id="mainNav">
        <div class="container">
            <a class="navbar-brand js-scroll-trigger" href="{{ route('login') }}"><img
                    src="{{ is_null(allsetting()['logo']) || allsetting()['logo'] == ''? asset(IMG_STATIC_PATH . 'logo.png'): asset(IMG_PATH . allsetting()['logo']) }}"
                    alt="{{ __('Nftzai') }}"></a>
            ...
            <div class="collapse navbar-collapse" id="navbarResponsive">
                <ul class="navbar-nav text-capitalize mr-auto">
                    <li class="nav-item">
                        <a class="nav-link {{ Route::is('discover') ? 'active' : '' }}"
                            href="{{ route('discover') }}">{{ __('Discover') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ Route::is('how-it-works') ? 'active' : '' }}"
                            href="{{ route('how-it-works') }}">{{ __('How
                            it works') }}</a>
                    </li>
                    <li class="nav-item">
                ...
                <div class="header-right-nav my-2 my-lg-0 d-flex">
                    <form class="searchbox" method="GET" action="#">
                        ...
                        <input type="text" placeholder="Search..." name="keyword" class="searchbox-input"
                            id="search-service" autocomplete="off">
                        <button type="submit" class="searchbox-submit searchbox-icon nav-search menu-round-btn"><i
                    ...
                    <ul class="header-right-menu-ul">
                        ...
                        @else
                            ...
                            <li class="nav-wallet"><a data-toggle="modal" href="#signInModal"
                                    class="theme-button2">{{ __('Join With Us') }}</a></li>
                        @endif
                        <!-- If isLoginMenu Then Show Menu User Box -->
                    </ul>
                </div>
                ...
                <div class="mobile-right-side">
                    <ul class="header-action-link d-flex align-items-center">
                        @if (Auth::check())
                            <!-- If isLoginMenu Then Show Menu User Box -->
                            <li class="nav-item dropdown">
                                <button class="dropdown-toggle d-flex align-items-center" id="userNavbarDropdown1"
                                    type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    <span class="navbar-user-img-wrap position-relative">
                                        <img src="{{ isset(Auth::user()->photo)? asset(IMG_USER_VIEW_PATH . Auth::user()->photo): Avatar::create(Auth::user()->first_name . ' ' . Auth::user()->last_name)->toBase64() }}"
                                            alt="{{ __('user') }}">
                                ...
                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="userNavbarDropdown1">
                                    ...
                                    <!-- Show If Wallet not connected after login -->
                                    <a href="{{ route('my_wallets') }}"
                                        class="theme-button1 user-dropdown-nav-wallet w-100">{{ __('My
                                                                                                                                                                                                            Wallets') }}</a>
                                    <!-- Show If Wallet not connected after login -->
                                    <a class="dropdown-item" href="{{ route('login') }}"><i
                                            class="fas fa-tachometer-alt"></i>{{ __('Dashboard') }}</a>
                                    ...
                                    <div class="dropdown-divider"></div>
                                    <a class="dropdown-item" href="{{ route('logOut') }}"><i
                                            class="fas fa-sign-in-alt"></i>{{ __('Disconnect') }}</a>
                                </div>
                            </li>
                            <!-- If isLoginMenu Then Show Menu User Box -->
                        @endif
        ...
        <div class="mobile-menu-bottom">
            ...
            <div class="header-right-nav">
                <form class="mobile-search position-relative w-100">
                    ...
                    <button type="button" class="nav-search menu-round-btn mobile-search-btn"><i
                            class="fas fa-search"></i></button>
                </form>
                <ul>
                    @if (Auth::check() != true)
                        <li><a data-toggle="modal" href="#signInModal"
                                class="theme-button1 nav-upload w-100">{{ __('upload') }}</a></li>
                        <li><a data-toggle="modal" href="#signInModal"
                                class="theme-button2 nav-wallet w-100">{{ __('Connect
                                                                                                                                                            Wallet') }}</a>
                        </li>
                    @else
                        <li><a href="{{ route('service_create') }}"
